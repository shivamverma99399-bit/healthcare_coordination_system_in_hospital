...
from .services import generate_unique_admin_identifier
...
class LoginSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if not user or not self._is_hospital_admin(user):
            raise serializers.ValidationError({"detail": "Invalid hospital admin credentials."})

        attrs["user"] = user
        return attrs

    def _is_hospital_admin(self, user):
        ...
        return bool(
            ...
            or (
                profile
                and profile.role == UserProfile.ROLE_HOSPITAL
    ...
    def _name_from_email(self, email):
        ...
        return cleaned.title() or "Patient"
    ...
    def _sync_patient_profile(self, user, full_name, city, phone, emergency_contact):
        ...
        PatientProfile.objects.update_or_create(
            ...
            defaults={
                ...
                "emergency_contact": emergency_contact or "Emergency Contact",
            },
        )
...
class AdminRegisterSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        with transaction.atomic():
            user = User.objects.create_user(
                username=self._build_admin_username(email),
                email=email,
                password=validated_data["password"],
                first_name=hospital_name[:150],
                last_name="Admin",
            )
            UserProfile.objects.create(
                user=user,
                role=UserProfile.ROLE_HOSPITAL,
    ...
    def _build_admin_username(self, email):
        local_part = email.split("@", 1)[0]
        candidates = [
            email,
            f"hospital__{email}",
            local_part,
            f"hospital__{local_part}",
        ]

        for raw_candidate in candidates:
            candidate = raw_candidate[:150]
            if candidate and not User.objects.filter(username=candidate).exists():
                return candidate

        suffix = 1
        while True:
            candidate = f"hospital{suffix}__{local_part}"[:150]
            if candidate and not User.objects.filter(username=candidate).exists():
                return candidate
            suffix += 1
...
class MedicalDocumentSerializer(serializers.ModelSerializer):
    ...
    def get_download_url(self, obj):
        relative_url = reverse("document-download", args=[obj.id])
    ...
    def get_can_delete(self, obj):
        ...
        if hasattr(user, "patient_profile"):
            return obj.source == "patient" and obj.patient_id == user.patient_profile.id

        if getattr(user, "is_superuser", False):
            return True

        admin_profile = getattr(user, "hospital_admin_profile", None)
        return bool(admin_profile and obj.booking and obj.booking.hospital_id == admin_profile.hospital_id)
...
class HospitalUpdateSerializer(serializers.Serializer):
    ...
    icu_beds = serializers.IntegerField(min_value=0)

    def update(self, instance, validated_data):
        beds = validated_data["beds"]
        icu_beds = validated_data["icu_beds"]

        instance.available_beds = beds
        instance.available_icu = icu_beds
        instance.total_beds = max(instance.total_beds, beds)
        instance.total_icu = max(instance.total_icu, icu_beds)
        instance.save(update_fields=["available_beds", "available_icu", "total_beds", "total_icu"])
        return instance

    def create(self, validated_data):  # pragma: no cover - serializer is update-only
        raise NotImplementedError("HospitalUpdateSerializer does not support create().")

    def to_representation(self, instance):
        return {
            "id": instance.id,
            "name": instance.name,
            "city": instance.city or instance.location,
            "beds": instance.available_beds,
            "icu_beds": instance.available_icu,
        }
...
class BookingRecordSerializer(serializers.ModelSerializer):
    ...
    def get_documents(self, obj):
        ...
        return MedicalDocumentSerializer(documents, many=True, context={"request": request}).data
